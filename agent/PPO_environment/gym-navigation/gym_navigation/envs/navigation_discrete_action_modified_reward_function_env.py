...
class navigation_discrete_action_modified_reward_functionEnv(gym.Env):
    metadata = {}
    ...
    def step(self, action):
        ...
        reach_high_reward_zone = []
        reach_low_reward_zone = []
        for bin_num in range(len(x_pos_range)):
            reach_high_reward_zone.append(0.0 <= x_pos_range[bin_num] <= self.reward_zone_size and -self.reward_zone_size <= y_pos_range[bin_num] <= self.reward_zone_size)
            reach_low_reward_zone.append(-self.reward_zone_size <= x_pos_range[bin_num] < 0.0 and -self.reward_zone_size <= y_pos_range[bin_num] <= self.reward_zone_size)

        reward = 0
        if done and True in reach_high_reward_zone and True in reach_low_reward_zone and np.argmax(reach_high_reward_zone) < np.argmax(reach_low_reward_zone):
            reward = 1.25
        elif done and True in reach_high_reward_zone and False in reach_low_reward_zone:
            reward = 1.25
        elif done and True in reach_high_reward_zone and True in reach_low_reward_zone and np.argmax(reach_low_reward_zone) < np.argmax(reach_high_reward_zone):
            reward = 0.125
        elif done and False in reach_high_reward_zone and True in reach_low_reward_zone:
            reward = 0.125
